...
def analyze_case_with_bert(text):
    model_path = "C:\Desktop\LA Final Project 1\Main_app\model\legalbert_petition_classifier"
...
def find_lawyers_route():
    ...
    if request.method == 'POST':
        ...
        if district and case_type:
            ...
            lawyer_df = pd.read_csv("C:\Desktop\LA Final Project 1\Main_app\data\LawyerDataFinal (1).csv")
...
def admin_login():
    if request.method == 'POST':
        ...
        csv_path = "C:\Desktop\LA Final Project 1\Main_app\data\LawyerDataFinal (1).csv"
...
client = Together(api_key="c1a5dafd01c6bdf2a58f93321f64a7fb6acfd0cfdfbb1526260bbebd237b94c4")
...
def clean_output(text):
    ...
    clean_lines = [
        re.sub(r'\*\*(.*?)\*\*', r'\1', line)
        for line in lines if not line.strip().startswith('<') and line.strip()
    ]
    return '\n'.join(clean_lines)

def summarize_petition(text):
    try:
        prompt = f"""
        Summarize the following legal petition in 2 to 4 lines only.
        Do not include any internal thoughts or explanation. Just provide the summary.
        Focus strictly on the petitioner's key facts, claims, and relief sought.
        Avoid legal jargon, citations, formatting, or reasoning.
...
def analyze_petition(text):
    try:
        prompt = f"""
        Analyze the following legal petition and provide:
        Max 5 Strong Points
        Max 5 Weak Points

        Return only bullet points under each heading.
        Do not include internal thoughts, introductions, or reasoning. No bold or extra formatting.
        ...
        return clean_output(raw_result)
    except Exception as e:
        print(f"Analysis error: {str(e)}")
        return None

@app.route('/analyze_petition', methods=['GET', 'POST'])
def analyze_petition_route():
    ...
    if request.method == 'POST':
        ...
        if 'petition_file' in request.files:
            file = request.files['petition_file']
            if file and file.filename.endswith('.pdf'):
                try:
                    petition_text = extract_text_from_pdf(file)
                    if not petition_text:
                        raise Exception("Could not extract text from PDF")
                except Exception as e:
                    error = f"Error processing PDF: {str(e)}"
                    flash(error, 'danger')
                    return render_template('analyze_petition.html', error=error)
        
        # If no PDF or PDF processing failed, try text input
        if not petition_text:
            petition_text = request.form.get('petition_text', '').strip()
            
        if not petition_text:
            flash('Please enter petition text or upload a PDF', 'warning')
            return render_template('analyze_petition.html')
        
        try:
            summary = summarize_petition(petition_text)
            analysis_result = analyze_petition(petition_text)
            
            if not summary or not analysis_result:
                raise Exception("Failed to generate analysis")
            
            # Save successful analysis to session
            ...
            flash('Petition analysis completed successfully', 'success')
        except Exception as e:
            error = f"Error analyzing petition: {str(e)}"
            flash(error, 'danger')
